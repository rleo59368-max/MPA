...
# -----------------------------
# Mostrar gráfica de personas con disposición radial
# -----------------------------
if categoria_seleccionada:
    ...
    if not filtro.empty:
        ...
        # Generar posiciones radiales
        angles = np.linspace(0, 2*np.pi, n, endpoint=False)
        for i, (_, row) in enumerate(filtro.iterrows()):
            # Radio proporcional a los votos (mínimo base + factor)
            r = base_radius + factor_votos * row["Votos"]
            x = 0.5 + r * np.cos(angles[i])
            y = 0.5 + r * np.sin(angles[i])
            size = 30 + row["Votos"]*5  # tamaño visible
            ...
            if pd.notna(row["Imagen"]) and row["Imagen"]:
                ...
                # Solo contorno si no hay imagen
                fig.add_trace(go.Scatter(
                    x=[x],
                    y=[y],
                    mode="markers+text",
