...
if categoria_seleccionada:
    ...
    if not filtro.empty:
        np.random.seed(42)
        # Generar posiciones más cercanas
        filtro["x"] = np.random.rand(len(filtro))*0.8 + 0.1
        filtro["y"] = np.random.rand(len(filtro))*0.8 + 0.1

        fig = go.Figure()
        for _, row in filtro.iterrows():
            # Tamaño proporcional a votos (escala aumentada)
            size = 30 + row["Votos"]*5

            if pd.notna(row["Imagen"]) and row["Imagen"]:
                # Imagen circular escalada según tamaño
                scale = size / 100
                ...
                # marcador transparente para hover
                ...
                # Si no hay imagen, solo contorno del círculo
                fig.add_trace(go.Scatter(
                    x=[row["x"]],
                    y=[row["y"]],
                    mode="markers+text",
                    marker=dict(size=size, color="rgba(0,0,0,0)", line=dict(width=3, color='DarkSlateGrey')),
                    text=[row["Votos"]],
