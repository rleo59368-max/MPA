...
def convertir_a_base64(imagen_file):
...
with st.sidebar.form("form_persona"):
    ...
    if submit and nombre:
        df = cargar_datos()
        if imagen_file:
            img_uri = convertir_a_base64(imagen_file)
        elif imagen_url:
            img_uri = imagen_url
        else:
            img_uri = ""
...
# -----------------------------
# Selector de categoría para visualización
# -----------------------------
...
# -----------------------------
# Gráfica de globos con imágenes dentro
# -----------------------------
if not filtro.empty:
    np.random.seed(42)
    ...
    for _, row in filtro.iterrows():
        size = 20 + row["Votos"]*2  # tamaño proporcional a votos
        # Círculo principal transparente para que se vea la imagen
        fig.add_trace(go.Scatter(
            x=[row["x"]],
            y=[row["y"]],
            mode="markers+text",
            marker=dict(size=size, color="rgba(0,0,0,0)", line=dict(width=2, color='DarkSlateGrey')),
            text=[row["Votos"]],
